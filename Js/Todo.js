...
function emptyMassage() {
    ...
    if (count === 0) {
        let x = document.createTextNode("Not Task");
        p.append(x);
...
//     tasks.style.cssText = "display:flex; flex-direction:column; width:400px; padding:20px; background-color:#eee; width:600px; border-radius:5px; ";
...
//     let data = JSON.parse(localStorage.getItem("task")) === null ? {} : JSON.parse(localStorage.getItem("task"));
//     let rand = (Math.random() * Date.now().toString()).toFixed();
//     if (userInput.trim() !== "") {
//         console.log("ss");
//         let obj = {
//             "id": rand,
...
// let tasksElement = JSON.parse(localStorage.getItem("task"));
